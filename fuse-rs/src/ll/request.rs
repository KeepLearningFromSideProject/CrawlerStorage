...
const FUSE_HEADER_SIZE: usize = mem::size_of::<fuse_in_header>();
...
impl<'a> fmt::Display for Operation<'a> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Operation::IoCtl {arg, ..} => write!(f, "IOCTL cmd {}", arg.cmd),
            Operation::Destroy => write!(f, "DESTROY"),
            Operation::BatchForget { .. } => write!(f, "BATCHFORGET"),
            Operation::NotifyReply {..} => write!(f, "NOTIFYREPLAY"),
            Operation::Poll {..} => write!(f, "POLL"),
            Operation::FAllocate {..} => write!(f, "FALLOCATE"),
